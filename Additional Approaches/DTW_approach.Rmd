...
```{r}
pacman::p_load(tidyverse, dtwclust, tictoc, TSclust)
```
...
```{r}
tslst <- SPdata_reduced %>% lapply(dfConvertr) 
```
...
pc <- tsclust(tslst, type = "partitional", k = 20L, 
...
series <- reinterpolate(tslst, new.length = max(lengths(CharTraj)))

# Subset for speed
series <- series[1:2000]
labels <- CharTrajLabels[1:20]

# Making many repetitions

tic()
pc.l2 <- tsclust(series, k = 14L,
                 distance = "L2", centroid = "pam",
                 seed = 3247, trace = TRUE,
                 control = partitional_control(nrep = 11L))
toc()
...
pc.dtw20 <- tsclust(tslst, k = 20L, seed = 3251, trace = TRUE,
                  args = tsclust_args(dist = list(window.size = 18L)))
...
tic()
## Use constrained DTW and PAM (less than a second with 8 cores)
pc.dtw15 <- tsclust(tslst, k = 15L, seed = 3251, trace = TRUE,
                  args = tsclust_args(dist = list(window.size = 18L)))
...
tic()
## Use constrained DTW and PAM (less than a second with 8 cores)
pc.dtw10 <- tsclust(tslst, k = 10L, seed = 3251, trace = TRUE,
                  args = tsclust_args(dist = list(window.size = 18L)))
...
#### Try other variant using Montero Mansos Package TSClust
